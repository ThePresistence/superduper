...
c----------------------------------------------------------------------- 
      ...
      IMPLICIT NONE
      integer n,ia(n+1),iao(n+1),ja(*),jao(*),job,ipos
      DOUBLE PRECISION a(*),ao(*)
c-----------------------------------------------------------------------
...
c
         do while (ii+irow .le. nrow .and. irow .le. m-1) 
            do 13 k=ia(ii+irow),ia(ii+irow+1)-1
c     
...
c--------loop on first row in block row to determine block sparsity
         j = 0
         do 37 jj = ia(kvstr(i)), ia(kvstr(i)+1)-1
            jnew = iwk(ja(jj))
            if (jnew .ne. j) then
...
c--------loop on all the elements in the block row to determine block sparsity
         do 43 jj = ia(kvstr(i)), ia(kvstr(i+1))-1
...
c-----------loop on block columns
            do 52 j = ib(i), ib(i+1)-1
c--------------loop on scalar columns within block column
               do 51 jj = kvstc(jb(j)), kvstc(jb(j)+1)-1
c-----------------check there is enough space in b array
                  if (b0 .gt. nzmax) then
                     ierr = 2
...
c--------construct ja for a scalar row
         do 57 j = ib(i), ib(i+1)-1
            do 56 jj = kvstc(jb(j)), kvstc(jb(j)+1)-1
               ja(a0) = jj
               a0 = a0 + 1
 56         continue
 57      continue
