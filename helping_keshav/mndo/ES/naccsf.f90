...
!**********************
!
SUBROUTINE NAC_OVLP(ONEDEN,CG,CC,DELTAE,LDA,NATOMS,NP)
  IMPLICIT NONE
  INTEGER  :: LDA, NATOMS, NP
  REAL*8   :: DELTAE(NP)
  REAL*8   :: ONEDEN(LDA,LDA,NP), CG(3,NATOMS,NP), CC(3,NATOMS,NP)
  
  CALL NACCSF_INIT

  IF(IAPP.NE.0) THEN
     CALL NACCSF_FD(ONEDEN,CG,CC,DELTAE,LDA,NP)
  ELSE
     CALL NACCSF_APPROX(ONEDEN,CG,CC,DELTAE,LDA,NP)
  ENDIF
...
!
SUBROUTINE NACCSF_FD(ONEDEN,CG,CC,DELTAE,LM3,NP)
  ...
  IMPLICIT NONE
  INTEGER  :: IA, IC, I, J, K, LM3, NP
  REAL*8   :: DELTAE(NP), tmp, Q0
  REAL*8   :: ONEDEN(LM3,LM3,NP)
  REAL*8   :: CG(3,NUMAT,NP)
  REAL*8   :: CC(3,NUMAT,NP)
  ...
  DO IA=1,NUMAT
     DO IC=1,3
        ...
        DO K=1,NP
           ...
           DO I=1,LM3
              DO J=1,LM3
                 TMP = TMP + ONEDEN(I,J,K) * 0.5D0 * (SIGMA(I,J) - SIGMA(J,I))
            END DO
           END DO
           CC    (IC,IA,K) = CG(IC,IA,K)/DELTAE(K) + TMP
        ENDDO
     END DO
  END DO
...
!  
SUBROUTINE NACCSF_APPROX(ONEDEN,CG,CC,DELTAE,LM3,NP)
  !(PARTIALLY) ANALYTICAL EVALUATION OF THE SECOND TERM (MO COEFFICIENT
  !AND AO DERIVATIVES) FOR NON-ADIABATIC COUPLING ELEMENTS.
  !ARGUMENTS:
  !ONEDEN: ONE-PARTICLE TRANSITION DENSITY MATRIX (I).
  ...
  DO IA=1,NUMAT
     DO IC=1,3
        ...
        ENDIF
        !DIVIDE BY ENERGY DIFFERENCE.
        TERM1(IC,IA) = CG(IC,IA,1) / DELTAE(1)
        ...
        DO K=1,NP
           ...
           DO I=1,LM3
              DO J=1,LM3
                 TMP = TMP + ONEDEN(I,J,K) * SIGMA(I,J) 
              END DO
           END DO
           CC    (IC,IA,K) = CG(IC,IA,K) / DELTAE(K) + TMP
        ENDDO
