...
( NR == 1 ) {
  ...
  nfinalstate1heat = split(finalstate1heat,afinalstate1heat,":") ;
  nfinalstate2heat = split(finalstate2heat,afinalstate2heat,":") ;
  ...
  nallcartgrd = split(allcartgrd,aallcartgrd,":") ;
  nallcartnac = split(allcartnac,aallcartnac,":") ;
  ...
  ncartnac = split(cartnac,acartnac,":") ;
  nintnac = split(intnac,aintnac,":") ;
  ...
  nmulstates = split(mulstates,amulstates,":") ;
  nmulheats = split(mulheats,amulheats,":") ;
  nmulqmnorms = split(mulqmnorms,amulqmnorms,":") ;
  nmulqmmmnorms = split(mulqmmmnorms,amulqmmmnorms,":") ;
  nmuloptnorms = split(muloptnorms,amuloptnorms,":") ;
  ngicvstates = split(gicvstates,agicvstates,":") ;
  ngicvqmnorms = split(gicvqmnorms,agicvqmnorms,":") ;
  ngicvqmmmnorms = split(gicvqmmmnorms,agicvqmmmnorms,":") ;
  ngicvoptnorms = split(gicvoptnorms,agicvoptnorms,":") ;
}
...
/FINAL STATE   1 HEAT OF FORMATION *[0-9\.+-][0-9\.+-]* *KCAL\/MOL/{
 calcfinalstate1heat[++ifinalstate1heat] = $7 ; next ; }
/FINAL STATE   2 HEAT OF FORMATION *[0-9\.+-][0-9\.+-]* *KCAL\/MOL/{
 calcfinalstate2heat[++ifinalstate2heat] = $7 ; next ; }
...
/ALL-ATOM CARTESIAN GRADIENT NORM/ { allcalccartgrd[++iallcartgrd] = $5 ; }
/   CARTESIAN GRADIENT NORM  /{ calccartgrd[++icartgrd] = $4 ; next ; }
/CARTESIAN NACV NORM/{ calccartnac[++icartnac] = $4 ; next ; }
/NACV NORM \(QM ATOMS ONLY\)/ {calcintnac[++iintnac] = $6; next ; }
/ALL-ATOM CARTESIAN NAC NORM/ { allcalccartnac[++iallcartnac] = $5 ; }
...
/SUMMARY OF MULTIPLE CI CALCULATIONS/{ getline; getline; getline;
  if ($6 == "QM+MM") {
    tabletype = 2; # QM+MM norms in col 5, opt norms in col 6
  } else if ($6 == "NORM") {
    tabletype = 1; # No QM+MM norms, opt norms in col 5
  } else {
    tabletype = 0; # Only energies, no norms (keep norms from previous tables)
  }
  getline;
  for (i = 1; i<=nmulstates; i++) {
    getline;
    calcmulstates[i] = $2;
    calcmulheats[i] = $3;
    if (tabletype > 0) {
	...
	calcmulqmmmnorms[i] = $5;
	calcmuloptnorms[i] = $6;
      }
    }
  }
}
/SUMMARY OF INTERSTATE COUPLING CALCULATIONS/{ getline; getline; getline;
  if ($6 == "QM+MM") {
    tabletype = 2; # QM+MM norms in col 5, opt norms in col 6
  } else {
    tabletype = 1; # No QM+MM norms, opt norms in col 5
  ...
  getline;
  for (i = 1; i<=ngicvstates; i++) {
    getline;
    calcgicvstates[i] = $2 "," $3;
    calcgicvqmnorms[i] = $4;
    if (tabletype == 1) {
      calcgicvoptnorms[i] = $5;
    } else {
      calcgicvqmmmnorms[i] = $5;
      calcgicvoptnorms[i] = $6;
    }
  }
}
END { 
  ...
  tolgradnrm    = 0.000011  ; # kcal/mol/angstrom
  tolnacnrm     = 0.000011  ; # kcal/mol/angstrom
  ...
  for ( i = 1 ; i <= nfinalstate1heat ; i++ ) {
    if( afinalstate1heat[i] != "" && (calcfinalstate1heat[i] < afinalstate1heat[i] - tolciheat ||
				calcfinalstate1heat[i] > afinalstate1heat[i] + tolciheat ) ){
      bad = bad sprintf("Wrong state 1 final heat of formation: %7.5f instead of %7.5f\n",
			calcfinalstate1heat[i],afinalstate1heat[i]) ;
    }
  }
  for ( i = 1 ; i <= nfinalstate2heat ; i++ ) {
    if( afinalstate2heat[i] != "" && (calcfinalstate2heat[i] < afinalstate2heat[i] - tolciheat ||
				calcfinalstate2heat[i] > afinalstate2heat[i] + tolciheat ) ){
      bad = bad sprintf("Wrong state 2 final heat of formation: %7.5f instead of %7.5f\n",
			calcfinalstate2heat[i],afinalstate2heat[i]) ;
    }
  }
  for ( i = 1 ; i <= nciheat ; i++ ) {
  ...
  for ( i = 1 ; i <= nallcartgrd ; i++ )
  {
    if( aallcartgrd[i] != "" && (allcalccartgrd[i] < aallcartgrd[i] - tolgradnrm ||
                                 allcalccartgrd[i] > aallcartgrd[i] + tolgradnrm) )
    {
      if (i > 1)  bad = bad sprintf("Molecule %d: ", i);
      bad = bad sprintf("Wrong all-atom cartesian gradient norm: %7.5f instead of %7.5f\n",
                       allcalccartgrd[i],aallcartgrd[i]) ;
    }
  }
  for ( i = 1 ; i <= ncartnac ; i++ )
  {
    if( acartnac[i] != "" && (calccartnac[i] < acartnac[i] - tolnacnrm ||
                              calccartnac[i] > acartnac[i] + tolnacnrm) )
    {
      if (i > 1)  bad = bad sprintf("Molecule %d: ", i);
      bad = bad sprintf("Wrong cartesian NACV norm: %7.5f instead of %7.5f\n",
                        calccartnac[i],acartnac[i]) ;
    }
  }
   for ( i = 1 ; i <= nallcartnac ; i++ )
  {
    if( aallcartnac[i] != "" && (allcalccartnac[i] < aallcartnac[i] - tolgradnrm ||
                                 allcalccartnac[i] > aallcartnac[i] + tolgradnrm) )
    {
      if (i > 1)  bad = bad sprintf("Molecule %d: ", i);
      bad = bad sprintf("Wrong all-atom cartesian coupling norm: %7.5f instead of %7.5f\n",
                       allcalccartnac[i],aallcartnac[i]) ;
    }
  }
  for ( i = 1 ; i <= nintgrd ; i++ )
  {
  ...
  for ( i = 1 ; i <= nintnac ; i++ )
  {
    if( aintnac[i] != "" && (calcintnac[i] < aintnac[i] - tolnacnrm ||
                             calcintnac[i] > aintnac[i] + tolnacnrm) )
    {
      if (i > 1)  bad = bad sprintf("Molecule %d: ", i);
      bad = bad sprintf("Wrong NACV norm: %7.5f instead of %7.5f\n",
                        calcintnac[i],aintnac[i]) ;
    }
  }
  ...
  for ( i = 1 ; i <= nmulstates ; i++ ) {
    if( amulstates[i] != "" && (calcmulstates[i] != amulstates[i] ) ){
      bad = bad sprintf("Wrong state calculated: %2d instead of %2d\n",
			calcmulstates[i],amulstates[i]) ;
    }
  }
  for ( i = 1 ; i <= nmulheats ; i++ ) {
    if( amulheats[i] != "" && (calcmulheats[i] < amulheats[i] - tolciheat ||
				calcmulheats[i] > amulheats[i] + tolciheat ) ){
      bad = bad sprintf("Wrong state %d heat of formation: %7.5f instead of %7.5f\n",
			amulstates[i],calcmulheats[i],amulheats[i]) ;
    }
  }
  for ( i = 1 ; i <= nmulqmnorms ; i++ ) {
    if( amulqmnorms[i] != "" && (calcmulqmnorms[i] < amulqmnorms[i] - tolgradnrm ||
				calcmulqmnorms[i] > amulqmnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong state %d cartesian QM gradient norm: %7.5f instead of %7.5f\n",
			amulstates[i],calcmulqmnorms[i],amulqmnorms[i]) ;
    }
  }
  for ( i = 1 ; i <= nmulqmmmnorms ; i++ ) {
    if( amulqmmmnorms[i] != "" && (calcmulqmmmnorms[i] < amulqmmmnorms[i] - tolgradnrm ||
				calcmulqmmmnorms[i] > amulqmmmnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong state %d cartesian QM+MM gradient norm: %7.5f instead of %7.5f\n",
			amulstates[i],calcmulqmmmnorms[i],amulqmmmnorms[i]) ;
    }
  }
  for ( i = 1 ; i <= nmuloptnorms ; i++ ) {
    if( amuloptnorms[i] != "" && (calcmuloptnorms[i] < amuloptnorms[i] - tolgradnrm ||
				calcmuloptnorms[i] > amuloptnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong state %d optimization gradient norm: %7.5f instead of %7.5f\n",
			amulstates[i],calcmuloptnorms[i],amuloptnorms[i]) ;
    }
  }
  for ( i = 1 ; i <= ngicvstates ; i++ ) {
    if( agicvstates[i] != "" && (calcgicvstates[i] != agicvstates[i] ) ){
      bad = bad sprintf("Wrong interstate coupling calculated: %s instead of %s\n",
			calcgicvstates[i],agicvstates[i]) ;
    }
  }
  for ( i = 1 ; i <= ngicvqmnorms ; i++ ) {
    if( agicvqmnorms[i] != "" && (calcgicvqmnorms[i] < agicvqmnorms[i] - tolgradnrm ||
				calcgicvqmnorms[i] > agicvqmnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong interstate coupling %s cartesian QM gradient norm: %7.5f instead of %7.5f\n",
			agicvstates[i],calcgicvqmnorms[i],agicvqmnorms[i]) ;
    }
  }
  for ( i = 1 ; i <= ngicvqmmmnorms ; i++ ) {
    if( agicvqmmmnorms[i] != "" && (calcgicvqmmmnorms[i] < agicvqmmmnorms[i] - tolgradnrm ||
				calcgicvqmmmnorms[i] > agicvqmmmnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong interstate coupling %s cartesian QM+MM gradient norm: %7.5f instead of %7.5f\n",
			agicvstates[i],calcgicvqmmmnorms[i],agicvqmmmnorms[i]) ;
    }
  }
  for ( i = 1 ; i <= ngicvoptnorms ; i++ ) {
    if( agicvoptnorms[i] != "" && (calcgicvoptnorms[i] < agicvoptnorms[i] - tolgradnrm ||
				calcgicvoptnorms[i] > agicvoptnorms[i] + tolgradnrm ) ){
      bad = bad sprintf("Wrong interstate coupling %s optimization gradient norm: %7.5f instead of %7.5f\n",
			agicvstates[i],calcgicvoptnorms[i],agicvoptnorms[i]) ;
    }
  }
  if( bad != "" ){ 
    printf "%s", bad ; 
