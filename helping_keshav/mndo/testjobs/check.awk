...
( NR == 1 ) {
  ...
  norder2en = split(order2en,aorder2en,":") ;
  ndavcorr = split(davcorr,adavcorr,":") ;
  npoplecorr = split(poplecorr,apoplecorr,":") ;
}
...
/SECOND-ORDER ENERGY/{ calcorder2en[++iorder2en] = $8 ; next ; }
/DAVIDSON CORRECTION/{ calcdavcorr[++idavcorr] = $8 ; next ; }
/POPLE CORRECTION/{ calcpoplecorr[++ipoplecorr] = $8 ; next ; }
END { 
  toltoten      = 0.0001 ;  # kcal/mol
  ...
  tolorder2en   = 0.00001 ;   # kcal/mol
  toldavcorr    = 0.000001 ;   # kcal/mol
  tolpoplecorr  = 0.000001 ;   # kcal/mol
  ...
  for ( i = 1 ; i <= ntoten ; i++ ) {
    if( atoten[i] != "" && (calctoten[i] < atoten[i] - toltoten ||
      ...
      bad = bad sprintf("Wrong total energy: %g instead of %g\n",
  ...
  for ( i = 1 ; i <= nfinaltoten ; i++ ) {
    if( afinaltoten[i] != "" && (calcfinaltoten[i] < afinaltoten[i] - tolfinaltoten ||
      ...
      bad = bad sprintf("Wrong final total energy: %g instead of %g\n",
  ...
  for ( i = 1 ; i <= norder2en ; i++ ) {
    if ( aorder2en[i] != "" && ( calcorder2en[i] < aorder2en[i] - tolorder2en ||
				 calcorder2en[i] > aorder2en[i] + tolorder2en ) ){
      bad = bad sprintf("Wrong BWEN second-order energy: %11.7f instead of %11.7f\n",
			calcorder2en[i],aorder2en[i]) ;
    }
  } 
  for ( i = 1 ; i <= ndavcorr ; i++ ) {
    if ( adavcorr[i] != "" && ( calcdavcorr[i] < adavcorr[i] - toldavcorr ||
				calcdavcorr[i] > adavcorr[i] + toldavcorr ) ){
      bad = bad sprintf("Wrong BWEN Davidson correction: %11.7f instead of %11.7f\n",
			calcdavcorr[i],adavcorr[i]) ;
    }
  } 
  for ( i = 1 ; i <= npoplecorr ; i++ ) {
    if ( apoplecorr[i] != "" && ( calcpoplecorr[i] < apoplecorr[i] - tolpoplecorr ||
				  calcpoplecorr[i] > apoplecorr[i] + tolpoplecorr ) ){
      bad = bad sprintf("Wrong BWEN Pople correction:    %11.7f instead of %11.7f\n",
			calcpoplecorr[i],apoplecorr[i]) ;
    }
  } 
  if( bad != "" ){ 
    printf "%s", bad ; 
