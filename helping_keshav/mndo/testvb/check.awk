...
( NR == 1 ) {
  ...
  nvbheat = split(vbheat,avbheat,":") ;
  nenuclr = split(enuclr,aenuclr,":") ;
...
/VB HEAT OF FORMATION/{ calcvbheat[++ivbheat] = $5 ; next ; }
/ENUCLR/{ calcenuclr[++ienuclr] = $2 ; next ; }
...
# Standard criteria from directory testjobs.
  ...
  tolvbheat     = 0.0002 ;  # kcal/mol
  tolenuclr     = 0.0000000000001 ;
  ...
  for ( i = 1 ; i <= nvbheat ; i++ ) {
    if( avbheat[i] != "" && (calcvbheat[i] < avbheat[i] - tolvbheat ||
			     calcvbheat[i] > avbheat[i] + tolvbheat ) ){
      bad = bad sprintf("Wrong VB heat of formation: %7.5f instead of %7.5f\n",
			calcvbheat[i],avbheat[i]) ;
    }
  }
  for ( i = 1 ; i <= nenuclr ; i++ ) {
    if ( aenuclr[i] != "" && ( calcenuclr[i] < aenuclr[i] - tolenuclr ||
			       calcenuclr[i] > aenuclr[i] + tolenuclr ) ){
      bad = bad sprintf("Wrong nuclear repulsion energy: %14.12f instead of %14.12f\n",
			calcenuclr[i],aenuclr[i]) ;
    }
  } 
  for ( i = 1 ; i <= noptcyc ; i++ ) {
