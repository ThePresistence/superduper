...
C     *
C     ONE-CENTER TWO-ELECTRON INTEGRALS FOR SPD-BASIS.
C     NI IS THE ATOMIC NUMBER OF THE CURRENT ELEMENT.
C     *
C     INPUT FROM COMMON BLOCKS: DNBND, DPARM, DPARM2, DPARM7.
C     RESULTS IN COMMON BLOCKS: DPARM3, DPARM4, REP.
C     *
      USE LIMIT, ONLY: LMZ
...
C *** INITIALIZATION.
      ...
      EP     = ZPN(NI)
      ED     = ZDN(NI)
C *** SLATER-CONDON PARAMETERS (Rlij).
C     FIRST  DIGIT (l)  L QUANTUM NUMBER OF SLATER-CONDON PARAMETER.
C     SECOND DIGIT (i)  SS 1, SP 2, PP 3, SD 4, PD 5, DD 6 - ELECTRON 1.
C     SECOND DIGIT (j)  SS 1, SP 2, PP 3, SD 4, PD 5, DD 6 - ELECTRON 2.
...
C     SAVE SLATER-CONDON PARAMETERS.
C     F0SS(NI) = R011
C     F0SP(NI) = R013
C     F0PP(NI) = R033
C     F2PP(NI) = R233
C     G1SP(NI) = R122
      ...
      G1PD(NI) = R155
      G3PD(NI) = R355
C     KEEP PREDEFINED SLATER-CONDON PARAMETERS (IF REQUESTED).
      IF(JF0SD(NI).GT.0) THEN
         R016  = F0SD(NI)
      ELSE
         F0SD(NI) = R016
      ENDIF
      IF(JG2SD(NI).GT.0) THEN
         R244  = G2SD(NI)
      ELSE
         G2SD(NI) = R244
      ENDIF
C *** COMPUTE ONE-CENTER TWO-ELECTRON INTEGRALS
C     FROM THE SLATER-CONDON PARAMETERS.
C *** INTEGRALS INVOLVING ONLY SP ORBITALS.
      GSS(NI) = R011
      GPP(NI) = R033 + (FOUR/25.0D0)*R233
      GP2(NI) = R033 - (TWO /25.0D0)*R233
      HPP(NI) = (THREE/25.0D0)*R233
C     EVALUATE GSP AND HSP ONLY IF REQUESTED.
C     NORMALLY GSP AND HSP ARE TREATED AS INDEPENDENT PARAMETERS.
      IF(JGSP(NI).LE.0) GSP(NI) = R013
      IF(JHSP(NI).LE.0) HSP(NI) = R122/THREE
C *** INTEGRALS INVOLVING ALSO D ORBITALS.
      REPD( 1,NI) = R016
