      SUBROUTINE FMTRX (A,APT,LMA,LMAPT,ARRAY,LM5,ICALL,SCFCAL)
C     *
...
C *** RESTART SECTION.
      IF(MIDDLE.EQ.1) THEN
         CALL FORSAV (A,APT,CG,DMX,LMA,LMAPT,I3N,NCOL,-1,MIDDLE)
...
C     ESTIMATES FOR TIMING.
      IF(ISKPA.EQ.1 .AND. KPOINT.EQ.1) THEN
         TMA = TME*4*I3N
         TMT = TME*2*I3N*NNCOL
         IF(KPRINT.GT.0) WRITE(NB6,540) TME
      ELSE
         TMA = TME*2
         TMT = TMA*NNCOL
         IF(KPRINT.GT.0) WRITE(NB6,550) TME
      ENDIF
      IF(KPRINT.GT.0) WRITE(NB6,560) TMA,TMT
C     *
...
C     ICG(L)=-1  IF ELEMENT (L,ICOL) MUST BE ZERO.
  ...
  140    CONTINUE
      ENDIF
C     CHECK FOR SIMPLIFIED CALCULATION.
      IF(KPOINT.EQ.1 .AND. ICOL.GT.1) THEN
         DO 150 L=1,ICOL-1
         ICG(L) = -1
  150    CONTINUE
      ENDIF
...
C *** SAVE RESULTS ON FILE.
      CALL FORSAV (A,APT,CG,DMX,LMA,LMAPT,I3N,NCOL,ICOL,MIDDLE)
C *** CHECK FOR TIME LIMIT.
      IF(NCOL.LT.NNCOL) THEN
         IF(TME.GT.TMA) TMA=TME
         TESTIM = TMF+TMA*SFACT
         IF(TESTIM.GT.TLIMIT) THEN
            WRITE(NB6,600)
            ICALL = -8
            RETURN
         ENDIF
...
C *** PRINT SYMMETRIZED FORCE CONSTANT MATRIX.
      IF(KPRINT.GT.0) THEN
         ...
         CALL MATPRT (A,I3N,I3N,I3N,I3N)
      ENDIF
      IF(KPOINT.NE.1 .AND. KPRINT.GE.0) WRITE(NB6,630) DLMAX
...
C *** SAVE FINAL RESULTS ON FILE.
  ...
  540 FORMAT(///5X,'TIME FOR INITIAL EVALUATION OF THE ENERGY ',
     1          8X,F14.3,' SECONDS')
  ...
  600 FORMAT(///5X,'THE JOB APPROACHES ITS TIME LIMIT.',
     1       /  5X,'ALL RELEVANT INFORMATION HAS BEEN SAVED ON FILE.',
     2       /  5X,'CONTINUE THE CALCULATION IN A NEW JOB.'//)
