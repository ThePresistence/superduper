      SUBROUTINE DIIS (FA,FB,PA,PB,FDA,FDB,ED,BD,AD,X,LM2,LM4,N,
...
C     FA(LM4)    CURRENT FOCK MATRIX (I). RHF OR ALPHA.
C                UPDATED FOCK MATRIX (O). RHF OR ALPHA.
C     FB(LM4)    CURRENT FOCK MATRIX (I). BETA (UHF).
C                UPDATED FOCK MATRIX (O). BETA (UHF).
C     PA(LM4)    CURRENT DENSITY MATRIX (I). RHF OR ALPHA.
C     PB(LM4)    CURRENT DENSITY MATRIX (I). BETA (UHF).
...
C     LM4        LEADING DIMENSION OF F,P,FD,ED: LM4=N*(N+1)/2 (I).
...
C     *
C     MATRICES ARE STORED IN PACKED LINEAR FORM.
...
C     *
     ...
     ./NBFILE/ NBF(20)
     ./UHF   / UHF
      DIMENSION FA(LM4),PA(LM4),FDA(LM4,MXDIIS+1)
      DIMENSION FB(LM4),PB(LM4),FDB(LM4,MXDIIS+1)
...
C *** DEBUG PRINT.
      IF(NPRINT.GE.9) THEN
         ...
         WRITE(NB6,710)
         CALL VECPRT (PA,LM4,N)
      ENDIF
C *** INITIALIZATION.
      NDIIS  = NDIIS+1
      KDIIS  = MOD(NDIIS-1,MXDIIS)+1
C *** COMPUTE THE ERROR MATRIX ED = F*P - P*F
C     FORTRAN CODE USING LINEARLY PACKED MATRICES.
      IF(JDIIS.EQ.1 .OR. JDIIS.EQ.3) THEN
         CALL DSPMM (FA,PA,ED(1,KDIIS),LM4,N,INDX,0)
         IF(UHF) THEN
            CALL DSPMM (FB,PB,ED(1,KDIIS),LM4,N,INDX,1)
         ENDIF
C     LIBRARY CALLS USING SQUARE MATRICES WITH EXTRA BUFFER SPACE.
C     NOTE THAT F AND P ARE SYMMETRIC, HENCE P*F = (F*P)transposed.
C     THE ERROR MATRIX ED = F*P - P*F = F*P - (F*P)transposed
C     CAN THUS BE EVALUATED USING ONLY ONE MATRIX MULTIPLICATION.
...
C     THE CALL TO TRPDIF COMPUTES F*P - (F*P)transposed (ED).
      ELSE
         ...
         CALL SQUARE (PA,ED(1,KDIIS),N,LM2,LM4)
         CALL ZZERO  (N,N,FDA(1,KDIIS),LM2)
         CALL DGEMM  ('N','N',N,N,N,ONE,FA,LM2,ED(1,KDIIS),LM2,
     1                ZERO,FDA(1,KDIIS),LM2)
         CALL LINEAR (FA,FA,N,LM2,LM4)
         IF(UHF) THEN
            CALL SQUARE (FB,FB,N,LM2,LM4)
            CALL SQUARE (PB,ED(1,KDIIS),N,LM2,LM4)
            CALL DGEMM  ('N','N',N,N,N,ONE,FB,LM2,ED(1,KDIIS),LM2,
     1                   ONE,FDA(1,KDIIS),LM2)
            CALL LINEAR (FB,FB,N,LM2,LM4)
         ENDIF
...
C *** DEBUG PRINT.
  ...
  690 FORMAT(1X)
  700 FORMAT(// 1X,'INPUT FOCK MATRIX.')
  710 FORMAT(// 1X,'INPUT DENSITY MATRIX.')
      END
