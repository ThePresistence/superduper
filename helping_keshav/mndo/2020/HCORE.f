      SUBROUTINE HCORE (H,W,S,B,COR,HO,LM2,LM4,LM6,LM9)
C     *
...
C     TRANSFORM TWO-ELECTRON INTEGRALS TO MOLECULAR COORDINATES.
      ...
      IF(IORBS.LE.4 .AND. JORBS.LE.4) THEN
         CALL ROTATE (IW,JW,IP,JP,KR,RI,YY,W,W,LM6,LM9,IMODE)
         KR  = KW
      ELSE
         CALL ROTD  (WW,YY,IW,JW)
         IF(IMODE.GT.0) THEN
            CALL W2LIN (IW*JW,KR,WW,W,LM9)
         ELSE
...
C *** ORTHOGONALIZATION CORRECTIONS FOR RESONANCE INTEGRALS.
  ...
  130    CONTINUE
      ENDIF
C
C *** WRITE FINAL MNDO INTEGRAL RECORD, IF NECESSARY.
      IF(INTSUM.GT.LM9) WRITE(NB2) W
C *** STORE MNDO INTEGRALS IN SQUARE FORM, IF NECESSARY.
      IF(IMODE.LE.0) CALL WSTORE (W,LM6,0)
      RETURN
      END
