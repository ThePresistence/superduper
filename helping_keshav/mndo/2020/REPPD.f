      SUBROUTINE REPPD (NI,NJ,R,RI,CORE,W,LIMIJ,LIMKL,MODESP)
C     *
...
C     *
      ...
      ENDIF
C     *
C *** CORE-ELECTRON ATTRACTION INTEGRALS.
C     CORE(I,1) - ELECTRONS (IJ) AT ATOM A AND CORE OF ATOM B (NC=NJ).
C     CORE(I,2) - ELECTRONS (KL) AT ATOM B AND CORE OF ATOM A (NC=NI).
C     *
C     INDEX I IN CORE(I,1) AND CORE(I,2).
C     (S S )=1, (S P )=2, (P P )=3, (P+P+)=4
C     (D S )=5, (D P )=6, (D D )=7, (D+P+)=8, (D+D+)=9, (D#D#)=10.
C     NOTATION: P-SIGMA=P , P-PI=P+, D-SIGMA=D , D-PI=D+, D-DELTA=D#.
C     *
      IF(MODESP.EQ.1) THEN
         IFIRST = 8
      ELSE
         IFIRST = 1
      ENDIF
C     LOOP OVER THE TWO POSSIBLE CASES.
C     NC   ATOMIC NUMBER OF ATOM CARRYING THE CORE.
C     NE   ATOMIC NUMBER OF ATOM CARRYING THE ELECTRONS.
C     LE   LIMIT FOR L PAIR INDEX, 1 FOR S, 3 FOR SP, 6 FOR SPD.
C     SIG  SIGN FACTOR FOR INTEGRALS, ALWAYS POSITIVE EXCEPT FOR
C          INTERACTIONS BETWEEN (IJ) AT ATOM A INVOLVING ONE P-SIGMA
C          OR D-PI ORBITAL AND THE CORE OF ATOM B (CASE N=1).
      DO 380 N=1,2
      IF(N.EQ.1) THEN
         NC  = NJ
         NE  = NI
      ...
      ELSE
         ...
         SIG = ONE
      ENDIF
C     CHECK FOR EXTERNAL POINT CHARGE.
...
C     ADDITIVE TERM FOR CORE.
      QA     = PO(9,NC)
C     CHECK FOR NEGLECT OF PENETRATION INTEGRALS.
C     IF THE ADDITIVE TERMS FOR SS (PO(1,NC)) AND THE CORE (PO(9,NC))
C     ARE EQUAL, THE CORE-ELECTRON ATTRACTION INTEGRALS ARE EXPRESSED
C     IN TERMS OF THE TWO-ELECTRON INTEGRALS (SS,KL) AND (IJ,SS).
C     LCW(I) IS THE STANDARD PAIR INDEX CORRESPONDING TO I IN CORE(I,N).
  ...
  320       CONTINUE
         ...
         GO TO 380
      ENDIF
C     EVALUATE RELEVANT DISTANCES BETWEEN POINT CHARGES.
...
C        L1=0, L2=2, M=0, LABEL=6.
      ...
      ENDIF
C     CALCULATE INVERSE DISTANCES.
C     THE NUMERATOR IS CHOSEN SUCH THAT THE NUCLEAR CHARGES ARE
C     INCLUDED ALREADY AT THIS POINT, AS WELL AS THE CONVERSION TO EV.
      FACTOR = -TORENC*EV
      DO 340 I=IFIRST,ILAST
      X(I)   = FACTOR/SQRT(X(I))
  340 CONTINUE
C     EVALUATE SEMIEMPIRICAL MULTIPOLE-MULTIPOLE INTERACTIONS XM(LABEL).
C     EVALUATE THE UNIQUE CORE-ELECTRON ATTRACTION INTEGRALS CORE(I,N)
C     FROM XM(LABEL).
...
C        DD.
  ...
  380 CONTINUE
C     NB6 = NBF(6)
C     WRITE(NB6,'('' CORE:'',/(2X,10F12.5))') CORE
      RETURN
      END
